...
- Block creation is serialized with a lock (`ledger.py: _chain_lock`) after a
  real race condition surfaced during Sepolia testing — two overlapping
  requests both read "latest block" before either committed, forking the
  hash chain. `GET /ledger/verify` now also checks chain-link continuity
  (each block's `prevHash` against the previous block's `onchainHash`), not
  just per-block tamper, since that's what caught the fork.
- The Isolation Forest is trained on synthetic baseline telemetry generated
  ...
  A real evaluation against a published ICS dataset (HAI 20.07) lives in
  `../ml-evaluation/` — see its README for the honest result: a calibrated
  statistical baseline actually outperforms the Isolation Forest alone on
  real attack data, and combining both detectors catches the most.
- `ACTOR_PROFILES` in `threat_intel.py` are now grounded in real MITRE
  ATT&CK for ICS group reporting (Xenotime/G0088, Sandworm/G0034, Volt
  Typhoon/G1017, ALLANITE), with citations in the module's docstring —
  see there for sources and the reasoning behind each weight. The scoring
  method itself (sum matched-technique weight, normalize against the
  strongest match) is still a simplification worth flagging in your
  write-up; it can and does produce genuine ties (Xenotime/Sandworm tie on
  this project's demo scenario), which the API surfaces explicitly via
  `tied_with` rather than silently picking one.
