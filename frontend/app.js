...
async function refreshAll() {
  ...
  const [ledger, anomalies, mitreTechniques, verify] = await Promise.all([
    api('/ledger/blocks'),
    api('/anomalies'),
  ...
  state.mitreTechniques = mitreTechniques;
  state.verify = verify;
  ...
  if (state.attribution) {
    attrEl.innerHTML = Object.entries(state.attribution.scores).sort((a, b) => b[1] - a[1]).map(([actor, score]) => `
      <div class="attr-row">
        ...
        <div class="attr-bar-track"><div class="attr-bar-fill ${actor === state.attribution.leader ? 'leading' : ''}" style="width:${score}%"></div></div>
      </div>`).join('');
...
function renderMitre() {
  ...
  grid.innerHTML = state.mitreTechniques.map(t => `
    <div class="mitre-cell ${t.hit ? 'hit' : ''}">
