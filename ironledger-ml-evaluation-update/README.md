...
ml-evaluation/  real-dataset evaluation of the anomaly detector (see ml-evaluation/README.md)
```
...
3. **Dual-layer anomaly detection** (`backend/app/anomaly.py`) — hard
   ...
   consistency) plus a scikit-learn `IsolationForest`. Independently
   evaluated against a real published ICS security dataset (HAI 20.07) in
   `ml-evaluation/` — see that folder's README for the honest result:
   on this dataset, a calibrated per-feature threshold rule actually
   outperforms the Isolation Forest alone, and combining both detectors
   (this project's actual design) catches the most attack segments overall.
