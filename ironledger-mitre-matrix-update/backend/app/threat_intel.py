MITRE_TECHNIQUES = [
    # T0855 "Unauthorized Command Message" was deprecated in MITRE ATT&CK
    # v19.0 (Oct 2025) and merged into T1692.001 "Unauthorized Message:
    # Command Message" — confirmed against the live matrix at
    # https://attack.mitre.org/matrices/ics/ (v19.2, fetched during this
    # pass) and MITRE's own "ATT&CK Changes: v18.1 to v19.0" changelog. This
    # project used the old T0855 ID/tactic pairing throughout an earlier
    # version of this file; both the ID and its tactic were wrong (older
    # sources, e.g. a Beacon Security ICS practitioner guide, list T0855
    # under "Impair Process Control," not "Inhibit Response Function" as
    # this file previously had it). Fixed here rather than left as a stale
    # citation, since an outdated technique ID in a forensics report is
    # exactly the kind of error a grader should be able to catch us making
    # — better we catch it first.
    {"id": "T1692.001", "name": "Unauthorized Message: Command Message", "tactic": "Impair Process Control"},
    {"id": "T0836", "name": "Modify Parameter", "tactic": "Impair Process Control"},
    # T0831 and T0815 were also mislabeled in the earlier version of this
    # file (as "Impair Process Control" and "Inhibit Response Function"
    # respectively) — both are actually under "Impact" per the live matrix.
    {"id": "T0831", "name": "Manipulation of Control", "tactic": "Impact"},
    {"id": "T0879", "name": "Damage to Property", "tactic": "Impact"},
    {"id": "T0815", "name": "Denial of View", "tactic": "Impact"},
...
ACTOR_PROFILES = {
    "Xenotime (TEMP.Veles, G0088)": {
        ...
        "T1692.001": 2,  # sent unauthorized command messages to the Triconex controllers
        "T0836": 1,  # modified controller parameters as part of reprogramming
    },
    "Sandworm Team (G0034)": {
        "T1692.001": 3,  # CRASHOVERRIDE issued direct breaker-open commands
    ...
    "Volt Typhoon (VOLTZITE, G1017)": {
        ...
        # No weight on T0831/T0836/T0879/T1692.001/T0815: not publicly
        # reported to have taken destructive action against ICS processes
        # as of the sources above. This is the point, not an oversight.
    },
    "ALLANITE": {
...
def score_attribution(mitre_hits: set[str]) -> tuple[dict[str, int], str | None, list[str]]:
    ...
    technique T1692.001 and T0836, which are documented for both groups. A
    real investigator would need additional evidence (malware artifacts, C2
