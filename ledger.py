...
from ..forensics import recompute_offchain_hash, is_tampered, verify_chain_integrity
...
def verify_chain(db: Session = Depends(get_db)):
    ...
    length, verified, mismatches = verify_chain_integrity(db)
    return schemas.VerifyResult(chain_length=length, chain_verified=verified, mismatches=mismatches)
